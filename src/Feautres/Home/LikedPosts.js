...
export default function LikedPosts() {
  ...
  const parseHomeContent = (payload, image, name) => {
    const parseData = payload?.map((list) => ({
      ...
      profile_img: process.env.REACT_APP_BASEURL_IMAGE + list?.Post?.User?.photos_for_verification,
      ...
      totalCommments: parseInt(list?.Post?.PostContents[0]?.totalCommentOnThisPost),
      totalTags: list?.Post?.PostContents[0]?.totalTagsOnThisPost,
      userLikeByHeart:
        list?.Post?.PostContents[0]?.CurrentUserLikeByHeart === "0" ? false : true,
      userLikeByStar:
        list?.Post?.PostContents[0]?.CurrentUserLikeByStar === "0" ? false : true,
      userLikeByThumb:
        list?.Post?.PostContents[0]?.CurrentUserLikeByThumb === "0" ? false : true,
    }));
    return parseData;
  };
